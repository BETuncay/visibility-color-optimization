...
public:
	...
	D3D(HWND WindowHandle) : 
		_Device(NULL), 
		_ImmediateContext(NULL),
		_BsDefault(NULL), 
		_BsBlendBackToFront(NULL),
		_BsAdditive(NULL),
		_DsTestWriteOff(NULL), 
		_RsCullNone(NULL)
	{
		...
		// -----------------------------------------
		D3D11_BLEND_DESC bsAdd;
		ZeroMemory(&bsAdd, sizeof(D3D11_BLEND_DESC));
		bsAdd.AlphaToCoverageEnable = false;
		bsAdd.IndependentBlendEnable = false; // -> dont need to set all 8 render targets
		for (int i = 0; i < 8; ++i)
		{
			bsAdd.RenderTarget[i].BlendEnable = true;
			bsAdd.RenderTarget[i].SrcBlend = D3D11_BLEND_ONE;
			bsAdd.RenderTarget[i].DestBlend = D3D11_BLEND_ONE;
			bsAdd.RenderTarget[i].BlendOp = D3D11_BLEND_OP_ADD;
			bsAdd.RenderTarget[i].SrcBlendAlpha = D3D11_BLEND_ONE;
			bsAdd.RenderTarget[i].DestBlendAlpha = D3D11_BLEND_ONE;
			bsAdd.RenderTarget[i].BlendOpAlpha = D3D11_BLEND_OP_ADD;
			bsAdd.RenderTarget[i].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;
		}
		_Device->CreateBlendState(&bsAdd, &_BsAdditive);
	...
	~D3D()
	{
		...
		if (_BsAdditive)			_BsAdditive->Release();			_BsAdditive = NULL;
	...
	// Gets the default blend state. (Blending disabled)
	ID3D11BlendState* GetBsAdditive() { return _BsAdditive; }
